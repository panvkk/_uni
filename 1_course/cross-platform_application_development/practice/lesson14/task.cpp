...
public:
    ...
    // Перегрузка операторов ввода-вывода (текстовая сериализация)
...
private:
    ...
    // Поля для хранения числа, месяца и года
    unsigned char d, m;
    unsigned short y;
};
...
Date::Date(int d, int m, int y) : d(d), m(m), y(y) {
    ...
    int max_day = day_count[m - 1] + static_cast<int>((m == 2) && IsLeapYear());
    if (!(d >= 1 && d <= max_day))
...
// Арифметические операторы (упрощённая реализация)
...
std::ostream& operator<<(std::ostream& out, const Date& date) {
    out << std::setw(2) << std::setfill('0') << date.d << "."
        << std::setw(2) << std::setfill('0') << date.m << "."
        << std::setw(4) << std::setfill('0') << date.y;
    return out;
}
...
// ---------------- Функции сериализации/десериализации в бинарном виде ----------------
...
int main() {
    try {
        Date d1;
        std::string line;
        std::cout << "Введите исходную дату: \n";
        {
            char sep1, sep2;
            int day, month, year;
