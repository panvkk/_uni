...
public class Main extends Application {
    ...
    private void loadFiles() {
        ...
        URL xmlUrl = getClass().getClassLoader().getResource("library.xml");
        URL xsdUrl = getClass().getClassLoader().getResource("library.xsd");

        if (xmlUrl != null && xsdUrl != null) {
            // В реальном приложении лучше копировать ресурсы во внешнюю папку,
            // но для лабы можно читать напрямую, если не упаковано в JAR.
            // Для записи нам нужен путь к файлу на диске.
            try {
    ...
    private VBox createControlPanel() {
        ...
        searchField.setPromptText("Search (Title, Author, Year)..."); // Обновил подсказку
        ...
        searchField.textProperty().addListener((observable, oldValue, newValue) -> {
            filteredData.setPredicate(book -> {
                ...
                if (book.getTitle().toLowerCase().contains(lowerCaseFilter)) return true; // Ищем в названии
                if (book.getAuthor().toLowerCase().contains(lowerCaseFilter)) return true; // Ищем в авторе
                if (String.valueOf(book.getYear()).contains(lowerCaseFilter)) return true; // Ищем в годе
                if (book.getGenre().toLowerCase().contains(lowerCaseFilter)) return true; // Ищем в жанре
